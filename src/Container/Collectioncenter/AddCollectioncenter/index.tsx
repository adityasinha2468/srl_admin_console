...
const AddCollectioncenter = (props: Props) => {
 ...
 const [allUsers, setAllUsers] = useState <any>([])
  ...
  const getUserByRoles = (roleId:any) => {
    axios
    .post('/listusersbyrole', {role_id:roleId})
    .then(function (response) {
      if (response?.status === 200) {
        setUserRoles(response.data.data.map((a: any) => ({value: a.id, label: a.name})))
      }
    })
    .catch(function (error) {
      // handle error
